...
func (db *DB) GetPiece(
	...
	if err != nil {
		return models.PieceResponse{}, fmt.Errorf("query failed for piece with id %d: %w", pieceID, err)
	}
	...
	if err != nil {
		...
		case errors.Is(err, pgx.ErrNoRows):
			return models.PieceResponse{}, fmt.Errorf("piece with id %d not found: %w", pieceID, err)
		case errors.Is(err, pgx.ErrTooManyRows):
			return models.PieceResponse{}, fmt.Errorf("multiple pieces with id %d found: %w", pieceID, err)
		default:
			return models.PieceResponse{}, fmt.Errorf("failed to collect piece with id %d: %w", pieceID, err)
		}
	}

	return pieceRow.ToResponse(), nil
}

// TODO: Consider implementing get by composer (id)
func (db *DB) GetPieces(
	ctx context.Context,
) ([]models.PieceResponse, error) {
...
func (db *DB) CreatePiece(
	...
	// Validate request
	if len(req.Title) < 1 {
		return models.PieceResponse{}, ErrTitleNotFound
	}

	rows, err := db.pool.Query(ctx, query, req.Title, req.ComposerID)
	if err != nil {
		return models.PieceResponse{}, fmt.Errorf("query failed: %w", err)
	}

	pieceRow, err := pgx.CollectExactlyOneRow(rows, pgx.RowToStructByName[models.PieceRow])
	if err != nil {
		return models.PieceResponse{}, fmt.Errorf("failed to collect inserted piece: %w", err)
	}
...
func (db *DB) UpdatePiece(
	...
	pieceRow, err := pgx.CollectExactlyOneRow(rows, pgx.RowToStructByName[models.PieceRow])
	if err != nil {
		return models.PieceResponse{}, fmt.Errorf("failed to collect updated piece row: %w", err)
	}
