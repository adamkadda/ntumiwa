...
func (db *DB) GetVenue(
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return nil, fmt.Errorf("venue with id %d not found: %w", venueID, ErrResourceNotFound)
		}

		return nil, fmt.Errorf("failed to collect venue with id %d: %w", venueID, err)
	}

	venueResponse := venueRow.ToResponse()

	return &venueResponse, nil
}

func (db *DB) ListVenues(
	ctx context.Context,
) ([]models.VenueResponse, error) {
...
func (db *DB) CreateVenue(
	...
	if len(req.Address) < 1 {
		return nil, fmt.Errorf("address field required: %w", ErrIncompleteResource)
	}
	...
	venueRow, err := pgx.CollectExactlyOneRow(rows, pgx.RowToStructByName[models.VenueRow])
	if err != nil {
		return nil, fmt.Errorf("failed to collect inserted venue row: %w", err)
	}
...
func (db *DB) UpdateVenue(
	...
	rows, err := db.pool.Query(ctx, query, venueID, req.Address)
	if err != nil {
		return nil, fmt.Errorf("query failed: %w", err)
	}
	...
	venueRow, err := pgx.CollectExactlyOneRow(rows, pgx.RowToStructByName[models.VenueRow])
	if err != nil {
		return nil, fmt.Errorf("failed to collect updated venue row: %w", err)
	}
