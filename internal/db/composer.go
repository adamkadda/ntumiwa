...
func (db *DB) GetComposer(
	...
	if err != nil {
		return nil, fmt.Errorf("query failed for composer with id %d: %w", composerID, err)
	}

	composerRow, err := pgx.CollectExactlyOneRow(rows, pgx.RowToStructByName[models.ComposerRow])
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return nil, fmt.Errorf("composer with id %d not found: %w", composerID, ErrResourceNotFound)
		}

		return nil, fmt.Errorf("failed to collect composer with id %d: %w", composerID, err)
	}

	composerResponse := composerRow.ToResponse()

	return &composerResponse, nil
}
...
func (db *DB) CreateComposer(
	...
	// Validate request
	if req.ShortName == "" {
		return nil, fmt.Errorf("short name field required: %w", ErrIncompleteResource)
	}

	if req.FullName == "" {
		return nil, fmt.Errorf("full name field required: %w", ErrIncompleteResource)
	}
	...
	composerRow, err := pgx.CollectExactlyOneRow(rows, pgx.RowToStructByName[models.ComposerRow])
	if err != nil {
		return nil, fmt.Errorf("failed to collect composer: %w", err)
	}
...
func (db *DB) UpdateComposer(
	...
	composerRow, err := pgx.CollectExactlyOneRow(rows, pgx.RowToStructByName[models.ComposerRow])
	if err != nil {
		return nil, fmt.Errorf("failed to update composer with id %d: %w", composerID, err)
	}
